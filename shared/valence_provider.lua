...
function valence.loadProvider(fn)
   trace(fn)
   ...
   if iguana.isTest() then
      providerfilepath = '/chs/to_amplefi/provider/provider.txt'
   end
   ...
   for i = 2, #d do
      ...
      ld = ld:gsub('"','')
      if ld ~= '' then
         local q = '[{"FileType":"provider"},{"rowpos":"'..i..'"}'
         q = q..',{"FileName":"'..fn..'"},{"payload":"'..ld..'"}]'
         trace(q)
         queue.push{data=q}
      end
   end
   q = '[{"FileType":"provider"},{"rowpos":"rowend"}'
   q = q..',{"FileName":"'..fn..'"},{"payload":""}]'
   queue.push{data=q}
end

function valence.processProvider(jsn,Data)
   iguana.logInfo(iguana.channelName()..': ToChannel main start.')
   local fn = jsn[3].FileName
   if jsn[2].rowpos ~= 'rowend' then
      local d = jsn[4].payload:split('|')
      local sql = "SELECT * FROM cchhs.dbo.provider "
      sql = sql.."WHERE PROVIDER_ID = '"..d[1].."' "
      sql = sql.."AND GROUP_ID = '"..d[17].."' "
      sql = sql.."AND SERVICE_LOCATION_ID = '"..d[37].."' "
      sql = sql.."AND BUSINESS_UNIT_ID = '"..d[38].."'"
      sql = sql..";"
      ...
      local s,r = pcall(conn.execute,{
         ...
         -- Insert new provider
         local c,v = '',''
         local sql1 = "INSERT INTO cchhs.dbo.provider ("
         for i = 1, #valence.provider_hdr do
            if i > 1 then
               c = c..","
               v = v..","
            end
      ...
      elseif s and #r > 0 then
         ...
         local sql2 = "UPDATE cchhs.dbo.provider SET "
         local cnt = 0
         
         for j = 1, #valence.provider_hdr do
            if valence.provider_hdr[j] ~= 'PROVIDER_ID' or
               valence.provider_hdr[j] ~= 'GROUP_ID' or
               valence.provider_hdr[j] ~= 'SERVICE_LOCATION_ID' or
               valence.provider_hdr[j] ~= 'BUSINESS_UNIT_ID' then
               cnt = cnt + 1
               if cnt > 1 then
                  sql2 = sql2..","
               end
               sql2 = sql2..valence.provider_hdr[j].." = '"..amplefi.DbValue(d[j]).."'"
            end
         end
         sql2 = sql2.." WHERE PROVIDER_ID = '"..d[1].."'"
         sql2 = sql2.." AND GROUP_ID = '"..d[17].."'"
         sql2 = sql2.." AND SERVICE_LOCATION_ID = '"..d[37].."'"
         sql2 = sql2.." AND BUSINESS_UNIT_ID = '"..d[38].."'"
         sql2 = sql2..";"
         if cnt ~= 0 then
            if not iguana.isTest() then
   ...
   iguana.logInfo(iguana.channelName()..': ToChannel main end.')
end
